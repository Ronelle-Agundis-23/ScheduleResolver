...
class AiScheduleService extends ChangeNotifier {
  ...
  Future<void> analyzeSchedule(List<TaskModel> tasks) async {
    ...
    try {
      ...
      final prompt = ''' 
        You are an Expert Student scheduling. The user has provided the following tasks for their day in Json format:
        $taskJson
        
        Your job is to analyze these tasks, identify any overlaps or conflicts in their start and end times, and suggest a better balance schedule.
        ...
        List any scheduling conflicts or state if there are none.
        2. ### Ranked Tasks
  ...
  ScheduleAnalysis _parseResponse (String fullText) {
    ...
    for (var section in sections) {
      if (section.startsWith('Detected Conflicts')) conflicts = section.replaceFirst('Detected Conflict', '').trim();
